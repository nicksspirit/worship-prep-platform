...
async def authorize_api_key(
    ...
    *,
    ...
    api_key = str(req_api_key or "").strip()
    if not api_key:
        logger.warning(
    ...
    prefix = parse_api_key_prefix(api_key)
    if prefix is None:
        logger.warning(
            "API key auth failed: invalid key format (expected wpp_live_<hex>.<secret>)",
            reason="invalid_format",
            present_length=len(api_key),
            has_separator="." in api_key,
        )
    ...
    if not constant_time_compare(hash_api_key(api_key), stored_key.hashed_key):
        logger.warning(
    ...
    await IntegrationApiKey.objects.filter(pk=stored_key.pk).aupdate(last_used_on=current_dt)
    
    stored_key.last_used_on = current_dt
